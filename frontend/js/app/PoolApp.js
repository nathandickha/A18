...
export class PoolApp {
    ...
    this._spaLive = {
      ...
      lastInputTs: 0,
      previewStreamMs: 200,
      rebuildDebounceMs: 160,
      rebuildTimer: 0,
      channelRefreshMs: 120,
      channelTimer: 0,
      dirty: new Set(),
      tilesInFlight: false,
      tilesQueued: false
...
// PBR / Caustics integration should never hard-crash the app if a module fails
  ...
  async addSpa() {
    ...
    if (this.poolGroup) {
      updatePoolWaterVoid(this.poolGroup, this.spa);
      this._scheduleSpaChannelRefresh();
    }
...
setupPoolEditor() {
  ...
  setupSpaSliders() {
    ["spaLength", "spaWidth", "spaTopHeight"].forEach((id) => {
      ...
      const onDown = () => {
        this.captureUndoState("Spa edit");
        this._setSpaLiveDragging(true);
      };
      const onUp = () => this._setSpaLiveDragging(false);
      ...
      slider.addEventListener("input", (e) => {
        ...
        if (id === "spaTopHeight" && this.spa.userData?.isHalfwaySnapped) {
        ...
        this._spaLive.dirty.add(id);
        this._spaLive.lastInputTs = performance.now ? performance.now() : Date.now();
        this._scheduleSpaPreviewTick();
        this._scheduleSpaRebuildDebounced();
      });

      slider.addEventListener("change", () => {
        this._setSpaLiveDragging(false);
      });
    });
  }

  _setSpaLiveDragging(isDragging) {
    this._spaLive.dragging = !!isDragging;
    if (!this._spaLive.dragging) {
      this._flushSpaRebuildNow();
    }
  }

  _scheduleSpaRebuildDebounced() {
    if (this._spaLive.rebuildTimer) clearTimeout(this._spaLive.rebuildTimer);
    this._spaLive.rebuildTimer = setTimeout(() => {
      this._spaLive.rebuildTimer = 0;
      if (this._spaLive.dragging) return;
      this._flushSpaRebuildNow();
    }, this._spaLive.rebuildDebounceMs);
  }

  _scheduleSpaPreviewTick() {
    if (this._spaLive.previewRaf) return;

    const tick = (ts) => {
      this._spaLive.previewRaf = 0;

      const minDt = 1000 / Math.max(1, this._spaLive.previewFps || 20);
      if (ts - this._spaLive.lastPreviewTs < minDt) {
        this._spaLive.previewRaf = requestAnimationFrame(tick);
        return;
      }
      this._spaLive.lastPreviewTs = ts;

      const streaming = (ts - (this._spaLive.lastInputTs || 0)) < (this._spaLive.previewStreamMs || 200);
      if (this._spaLive.dirty.size && (this._spaLive.dragging || streaming)) {
        this._applySpaLivePreviewFromDirty();
        this._spaLive.previewRaf = requestAnimationFrame(tick);
      }
    };
  ...
  _applySpaLivePreviewFromDirty() {
    if (!this.spa) return;

    updateSpa(this.spa);

    // Keep spa water/spillover visually current while dragging, but defer the
    // more expensive tile re-apply + ground/channel refresh until debounce/release.
    if (this.poolGroup) {
      try { updatePoolWaterVoid(this.poolGroup, this.spa); } catch (_) {}
    }
  }

  _scheduleSpaChannelRefresh() {
    if (this._spaLive.channelTimer) clearTimeout(this._spaLive.channelTimer);
    this._spaLive.channelTimer = setTimeout(() => {
      this._spaLive.channelTimer = 0;
      if (this.poolGroup) {
        updateGroundVoid(this.ground || this.scene?.userData?.ground, this.poolGroup, this.spa);
      }
    }, this._spaLive.channelRefreshMs || 120);
  }

  async _applyTilesToSpaQueued() {
    if (!this.spa || !this.pbrManager) return;
    if (this._spaLive.tilesInFlight) {
      this._spaLive.tilesQueued = true;
      return;
    }

    this._spaLive.tilesInFlight = true;
    this._spaLive.tilesQueued = false;
    try {
      await this.pbrManager.applyTilesToSpa(this.spa);
    } finally {
      this._spaLive.tilesInFlight = false;
      if (this._spaLive.tilesQueued) this._applyTilesToSpaQueued();
    }
  }

  async _flushSpaRebuildNow() {
    if (this._spaLive.rebuildTimer) {
      clearTimeout(this._spaLive.rebuildTimer);
      this._spaLive.rebuildTimer = 0;
    }

    if (!this.spa || !this._spaLive.dirty.size) return;
