...
const SPA_WALL_CENTER_SNAP_THRESHOLD = 0.05; // 50mm normal-to-wall tolerance
const HALF_WALL_MIN_TOP_OFFSET = 0.05;
...
// --- Top offset setter ---
function syncSpaTopSliderUI(val) {
  if (typeof document === "undefined") return;
  const slider = document.getElementById("spaTopHeight");
  const output = document.getElementById("spaTopHeight-val");
  if (slider) slider.value = String(val);
  if (output) output.textContent = Number(val).toFixed(2) + " m";
}

export function setSpaTopOffset(val) {
  const spa = selectedSpa;
  const minTop = spa?.userData?.isHalfwaySnapped ? HALF_WALL_MIN_TOP_OFFSET : 0.0;
  SPA_TOP_OFFSET = Math.max(minTop, val);
  if (spa) {
    spa.userData.spaTopHeight = SPA_TOP_OFFSET;
    updateSpaWalls(spa);
    updateSpaSeats(spa);
    syncSpaTopSliderUI(SPA_TOP_OFFSET);
  }
}
...
export function snapToPool(spa) {
  ...
  let isHalfwaySnapped = false;

  if (close === "left") {
    const innerPos = minX + l / 2 + dynamicSnap + wallNudge;
    const centerPos = minX;
    isHalfwaySnapped = Math.abs(x - centerPos) <= SPA_WALL_CENTER_SNAP_THRESHOLD;
    spa.position.x = isHalfwaySnapped ? centerPos : innerPos;
  ...
  if (close === "front") {
    ...
    isHalfwaySnapped = Math.abs(y - centerPos) <= SPA_WALL_CENTER_SNAP_THRESHOLD;
    spa.position.y = isHalfwaySnapped ? centerPos : innerPos;
  ...
  spa.userData.isHalfwaySnapped = isHalfwaySnapped;
  if (isHalfwaySnapped && SPA_TOP_OFFSET < HALF_WALL_MIN_TOP_OFFSET) {
    SPA_TOP_OFFSET = HALF_WALL_MIN_TOP_OFFSET;
    spa.userData.spaTopHeight = SPA_TOP_OFFSET;
    updateSpaWalls(spa);
    updateSpaSeats(spa);
    syncSpaTopSliderUI(SPA_TOP_OFFSET);
